...
use crate::resource::{AreaMap, AreaMapCollection, GameStage, GameState};
use tcod::noise::*;
...
impl<'a> System<'a> for MapGenerator {
    type SystemData = (Write<'a, AreaMapCollection>, Write<'a, GameState>);
...
impl MapGenerator {
    fn generate(&mut self, region: Region, map: &mut AreaMap, state: &mut GameState) {
        ...
        println!(
            "Generating new map with world seed {} for region {:?}",
